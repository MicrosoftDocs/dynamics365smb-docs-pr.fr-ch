...
## <a name="get-started"></a>Commencer à utiliser l'extension Gestion des QR-factures
...
## Ajout d'informations de facturation aux QR-factures
...
## Comprendre la référence de paiement
...
### <a name="formats"></a>Formats pour les références QR et les références créditeur
...
## <a name="multiplebankaccounts"></a>Utilisation de plusieurs comptes bancaires comme émetteurs de QR-factures
...
### Ajout d'un QR-IBAN à un compte bancaire nouveau ou existant
...
## Numérisation et importation de QR-factures
...
### Réception d'une QR-facture par le biais d'un document entrant
...
Lorsque la QR-facture est numérisée ou importée dans le document achat, le montant, la référence de paiement ainsi que d'autres informations figurant sur la QR-facture sont ajoutés. Ces données serviront à valider le document achat avant sa comptabilisation. La comptabilisation est bloquée si le montant sur la commande ou la facture ne correspond pas à celui sur la QR-facture. Une validation se produit également si vous numérisez ou importez la QR-facture. Si vous téléchargez la QR-facture qui ne contient pas le **Montant**, par exemple si la QR-facture décodée a une ligne blanche où devrait se trouver le montant, après le téléchargement, la section **QR-facture** est affichée sur la page **Facture achat**. Le champ **Montant** de la section **QR-facture** peut être modifié et vous pouvez ajouter le montant approprié dans ce champ. Si une QR-facture téléchargée a une valeur dans le champ **Montant**, elle est automatiquement définie dans le champ **Montant** de la section **QR-facture**, mais le champ peut toujours être modifié.  

Si la référence de paiement est déjà utilisée sur une écriture comptable fournisseur pour un fournisseur, une erreur se produit. Les fournisseurs ne peuvent pas émettre plusieurs QR-factures avec la même référence de paiement. De même, une erreur se produit si la QR-facture et la référence de paiement ont déjà été importées vers un document achat ouvert.

### Réception d'une QR-facture par le biais d'une feuille achat
