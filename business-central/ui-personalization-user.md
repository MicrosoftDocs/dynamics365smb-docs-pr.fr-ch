...
## Commencez à personnaliser en utilisant le mode de personnalisation
...
|Masquez un élément actuellement affiché, comme un champ, une colonne dans une liste, une vignette, une action ou une pièce.|Sélectionnez l’élément, sélectionnez la pointe de flèche, puis sélectionnez <b>Masquer</b>.|En mode personnalisation, les actions masquées sont grisées avec du texte en italique et les parties cachées sont ombrées par des lignes diagonales. Les champs et les colonnes masqués ne sont pas indiqués directement sur la page, mais vous pouvez les localiser en utilisant le volet <b>Ajouter un champ à la page</b> ([en savoir plus sur les champs de travail](#fields)).<br><br>Lorsque vous quittez le mode personnalisation, tous les éléments disparaissent de la vue. Si le champ que vous masquez s’affiche également sur l’en-tête Raccourci lorsque ce dernier est réduit, le champ n’y apparaîtra plus.|
...
## <a name="Actions"></a>Personnaliser la barre d’action et les menus
...
|**Afficher**|Cette option s’affiche si l’action ou le groupe d’actions est masqué(e) (atténué(e)). En choisissant cette option, vous permettez à l’action ou au groupe d’actions de figurer dans la barre de navigation ou la barre d’actions.|

### Déplacer les actions et groupes d’actions
...
## <a name="Parts"></a>Personnaliser les pièces
...
## <a name="fields"></a> Utiliser des champs et des colonnes

Lors de la personnalisation d’une page, vous utilisez **Ajouter un champ au volet de la page** pour inclure les champs ou les colonnes de la page actuellement masqués. Pour ouvrir ce volet, sélectionnez l’action **+ Champ** en haut de la page. Contrairement à d’autres éléments masqués, les champs masqués ne sont pas indiqués sur la page elle-même en mode personnalisation. Cependant, vous pouvez identifier les champs masqués en utilisant le volet **Ajouter un champ à la page**.

Voici quelques directives générales à suivre lors de l’utilisation du volet **Ajouter un champ à la page** :

- Par défaut, le volet répertorie tous les champs masqués. Les champs masqués sont marqués par l’icône ![Affiche l’icône du champ masqué](media/hidden-icon.png "Affiche l’icône du champ caché") .
- Vous pouvez filtrer la liste pour afficher d’autres champs, par exemple, ceux actuellement affichés sur la page, en sélectionnant le bouton **Champs recommandés** au-dessus de la liste et en choisissant une option de filtre. Le nom du bouton change en fonction de l’option de filtre que vous choisissez.
...
- La sélection d’un champ dans la liste met en évidence son emplacement sur la page. Si le champ est actuellement masqué, son emplacement par conception est affiché dans un état ombré. 
...
Dans le volet **Ajouter un champ à la page**, il existe deux manières d’inclure un champ actuellement masqué sur la page :
...
> [!NOTE]
> Certains champs que vous ajoutez ne seront pas modifiables sur la page une fois la personnalisation terminée. Ces champs sont soit conçus à l’origine de cette façon, soit un administrateur [personnalisé](ui-personalization-manage.md) la page pour vous empêcher de les modifier.
...
## Conseils et autres points d’intérêts
