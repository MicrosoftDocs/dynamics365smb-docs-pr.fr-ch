...
Désormais, lorsque l’action de synchronisation est déclenchée, la tâche s’exécute au premier plan et si une erreur se produit, vous obtenez une boîte de dialogue d’erreur avec le lien **Copier les détails**. Utilisez ce lien pour copier des informations supplémentaires dans un éditeur de texte pour une analyse plus approfondie.
...
### Erreur : L’en-tête des ventes n’existe pas. Champs et valeurs d’identification : Document Type=’Quote’,No.=’YOU SHOPIFY STORE’

Pour calculer les prix, le connecteur Shopify crée un document de vente temporaire (devis) pour un client temporaire (code de magasin) et laisse la logique de calcul de prix standard faire son travail. C’est un scénario typique lorsqu’une extension tierce s’abonne à des événements dans la ligne de vente, mais ne vérifie pas que l’enregistrement est temporaire, de sorte que l’en-tête peut ne pas être accessible. Notre recommandation consiste à contacter le fournisseur de l’extension et à lui demander de modifier son code pour vérifier si les enregistrements sont temporaires. Dans certains cas, ajouter la méthode `IsTemporary` méthode au bon endroit suffit. Pour en savoir plus sur IsTemporary, accédez à [IsTemporary](/dynamics365/business-central/dev-itpro/developer/methods-auto/record/record-istemporary-method). 

Pour vérifier que le problème est causé par une extension tierce, utilisez le lien **Copier les informations dans le presse-papiers** dans le message d’erreur et copiez le contenu dans l’éditeur de texte. Les informations contiennent une **pile d’appels AL**, où la ligne supérieure est la ligne où l’erreur s’est produite. Voici un exemple de pile d’appels AL.
...
### Erreur : Gén. Groupe compta. marché doit comporter une valeur dans Client: ’YOU SHOPIFY STORE’. La valeur ne peut pas être zéro et le champ ne peut pas être vide
