...
# <a name="troubleshooting-the-shopify-and-business-central-synchronization" />Dépannage de la synchronisation entre Shopify et Business Central
...
## <a name="run-tasks-in-the-foreground" />Exécuter des tâches au premier plan
...
### <a name="to-enable-logging" />Pour activer la journalisation
...
### <a name="to-review-logs" />Pour vérifier les journaux
...
## <a name="data-capture" />Capture de données
...
## <a name="reset-sync" />Réinitialiser la synchronisation
...
## <a name="request-the-access-token" />Demander le jeton d’accès
...
## <a name="verify-and-enable-permissions-to-make-http-requests-in-a-non-production-environment" />Vérifier et activer les autorisations pour effectuer des requêtes HTTP dans un environnement hors production
...
## <a name="rotate-the-shopify-access-token" />Faire tourner le jeton d’accès Shopify
...
### <a name="in-shopify" />Dans Shopify
...
### <a name="in-includeprodshortincludesprodshortmd" />Dans [!INCLUDE[prod_short](../includes/prod_short.md)]
...
## <a name="known-issues" />Problèmes connus

### <a name="error-the-sales-header-does-not-exist-identification-fields-and-values-document-typequotenoyour-shopify-store" />Erreur : L’en-tête des ventes n’existe pas. Champs et valeurs d’identification : Document Type=’Quote’,No.=’YOUR SHOPIFY STORE’
...
### <a name="error-gen-bus-posting-group-must-have-a-value-in-customer-your-shopify-store-it-cannot-be-zero-or-empty" />Erreur : Gén. Groupe compta. marché doit comporter une valeur dans Client: ’YOUR SHOPIFY STORE’. La valeur ne peut pas être zéro et le champ ne peut pas être vide
...
### <a name="error-importing-data-to-your-shopify-shop-isnt-enabled-go-to-the-shop-card-to-enable-it" />Erreur : L’importation de données dans votre boutique Shopify n’est pas activée. Accédez à la fiche magasin pour l’activer
...
### <a name="error-oauth-error-invalidrequest-could-not-find-shopify-api-application-with-apikey" />Erreur : Oauth error invalid_request: Could not find Shopify API application with api_key
...
### <a name="error-internal-error-looks-like-something-went-wrong-on-our-end-request-id-xxxxxxxx-xxxx-xxxx-xxxx-xxxx" />Erreur : erreur interne. Il semble qu’un problème se soit produit de votre côté. Identifiant de la demande : XXXXXXXX-XXXX-XXXX-XXXX-XXXX
