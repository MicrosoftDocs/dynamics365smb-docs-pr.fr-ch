...
Lorsque vous créez un journal, vous pouvez ajouter des liens qui offrent un contexte à ses transactions. Lorsque vous validez le journal, [!INCLUDE [prod_short](includes/prod_short.md)] copie les liens vers le journal validé et les écritures comptables créées par le journal. Par exemple, fournir des liens peut faciliter la vie de votre auditeur. Si vous conservez des images de vos reçus de dépenses sur le site Sharepoint de votre entreprise, vous pouvez ajouter des liens vers les fichiers. Lorsque vous validez le journal pour soumettre vos dépenses, votre auditeur peut accéder rapidement aux fichiers de reçus.
