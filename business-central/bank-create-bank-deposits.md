...
Vous devez paramétrer une souche de numéros pour les dépôts bancaires, puis spécifier la souche dans le champ **Numéros de dépôt bancaire** sur la page **Paramètres ventes**. Pour plus d’informations sur les souches de numéros, accédez à [Création des souches de numéros](ui-create-number-series.md).

Sur la page **Paramètres ventes** également, pour valider des dépôts sous forme de montants forfaitaires plutôt que sous forme de lignes individuelles, activez le bouton bascule **Valider les montants des dépôts bancaires sous forme de somme forfaitaire**. La validation d’un dépôt sous forme de somme forfaitaire, qui crée une écriture bancaire pour le montant total du dépôt, peut faciliter le rapprochement bancaire.
...
Vous devrez également créer un traitement par lots pour le modèle. Pour créer un traitement par lots, sur la page **Modèles feuille comptabilité**, choisissez l’action **Lots**. Pour en savoir plus sur les lots, consultez [Utilisation de modèles de feuilles et de lots](ui-work-general-journals.md#use-journal-templates-and-batches).

## <a name="dimensions-on-bank-deposit-lines"></a>Axes analytiques sur les lignes de dépôt bancaire
...
1. Sélectionnez ![l’icône en forme d’Ampoule qui ouvre la fenêtre de recherche](media/ui-search/search_small.png "Dites-moi ce que vous voulez faire"),  entrez **Dépôts bancaires**, puis sélectionnez le lien associé.
...
4. Dans le champ **N° compte bancaire**, choisissez le compte bancaire sur lequel effectuer le dépôt.
    ...
    > [!TIP]
    > Vous pouvez vérifier que vous déposez sur le compte correct en utilisant les actions **Fiche compte** et **Écritures comptables compte** pour rechercher les écritures comptables pour le compte bancaire sélectionné. Par exemple, pour vérifier si des dépôts similaires ont été effectués sur le compte.
...
## <a name="find-posted-bank-deposits"></a>Rechercher des dépôts bancaires validés
...
## <a name="reverse-a-posted-bank-deposit"></a>Annuler un dépôt bancaire validé

Il existe plusieurs façons d’annuler un dépôt bancaire effectué :

* Sur le **Dépôts bancaires validés** page, choisissez le dépôt, puis choisissez le **Annuler la validation** action.
* Sur la page **Hist. trans. comptabilité**, identifiez l’historique du dépôt, puis choisissez l’action **Contrepasser l’historique des transactions**.
