...
### <a name="to-register-an-application-in-microsoft-entra-id-for-connecting-from-business-central-to-exchange-online"></a>Pour enregistrer une application dans Microsoft Entra ID pour la connexion de Business Central à Exchange Online

Les étapes suivantes supposent que vous utilisez Microsoft Entra ID pour gérer les identités et les accès. Pour plus d’informations, voir [Démarrage rapide : enregistrer une application avec la plateforme d’identité Microsoft](/azure/active-directory/develop/quickstart-register-app). 
